...
class core_curl_security_helper_testcase extends advanced_testcase {
    /**
     * Test for \core\curl\curl_security_helper::url_is_blacklisted().
     *
     ...
     */
    public function test_curl_security_helper_url_is_blacklisted($url, $blockedhosts, $allowedports, $expected) {
        $this->resetAfterTest(true);
        set_config('curlsecurityblockedhosts', $blockedhosts);
        set_config('curlsecurityallowedport', $allowedports);
        $this->assertEquals($expected, \core\curl\curl_security_helper::url_is_blacklisted($url));
    }

    /**
     * Data provider for test_curl_security_helper_url_is_blacklisted().
     *
     * @return array
     */
    ...
    /**
     * Test for \core\curl\curl_security_helper::security_enabled().
     *
     * @param string $blockedhosts the list of blocked hosts.
     * @param string $allowedports the list of allowed ports.
     * @param bool $expected the expected result.
     ...
     */
    public function test_curl_security_helper_security_enabled($blockedhosts, $allowedports, $expected) {
        $this->resetAfterTest(true);
        set_config('curlsecurityblockedhosts', $blockedhosts);
        set_config('curlsecurityallowedport', $allowedports);
        $this->assertEquals($expected, \core\curl\curl_security_helper::security_enabled());
    }

    /**
     * Data provider for test_curl_security_helper_security_enabled().
     *
     * @return array
     */
    ...
    /**
     * Test for \core\curl\curl_security_helper::host_is_blocked().
     *
    ...
    public function test_curl_security_helper_host_is_blocked($host, $blockedhosts, $expected) {
        $this->resetAfterTest(true);
        set_config('curlsecurityblockedhosts', $blockedhosts);
        $this->assertEquals($expected, \core\curl\curl_security_helper::host_is_blocked($host));
    }
    ...
    /**
     * Test for \core\curl\curl_security_helper::port_is_blocked().
     *
    ...
    public function test_curl_security_helper_port_is_blocked($port, $allowedports, $expected) {
        $this->resetAfterTest(true);
        set_config('curlsecurityallowedport', $allowedports);
        $this->assertEquals($expected, \core\curl\curl_security_helper::port_is_blocked($port));
    }
    ...
    /**
     * Test for \core\curl\curl_security_helper::get_blocked_url_string().
     */
    public function test_curl_security_helper_get_blocked_url_string() {
        $this->assertEquals(get_string('curlsecurityurlblocked', 'admin'),
                            \core\curl\curl_security_helper::get_blocked_url_string());
    }

    /**
     * Test for the protected function \core\curl\curl_security_helper::url_parse().
     *
     * @param string $url the URL to test.
     * @param array|bool $expected array of expected [host, port] values on success, or false on failure.
     * @dataProvider curl_security_url_parse_data_provider
     */
    public function test_curl_security_helper_url_parse($url, $expected) {
        $this->assertEquals($expected, phpunit_util::invoke_static_method('core\\curl\\curl_security_helper', 'url_parse', [$url]));
    }

    /**
     * Data provider for test_curl_security_helper_url_parse.
     *
     * @return array
     */
    public function curl_security_url_parse_data_provider() {
        // Default set, which will be returned in the event of a failed parsing.
        $defaults = ['scheme' => '', 'host' => '', 'port' => '', 'user' => '',
                     'pass' => '', 'path' => '', 'query' => '', 'fragment' => ''];
        return [
            ["http://localhost/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'path' => '/x.png'])
            ],
            ["http://localhost:80/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'port' => 80, 'path' => '/x.png'])
            ],
            ["https://localhost/x.png",
                array_merge($defaults, ['scheme' => 'https', 'host' => 'localhost', 'path' => '/x.png'])
            ],
            ["http://localhost:443/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'port' => 443, 'path' => '/x.png'])
            ],
            ["localhost",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost'])
            ],
            ["localhost/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'path' => '/x.png'])
            ],
            ["sub.domain.com/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'sub.domain.com', 'path' => '/x.png'])
            ],
            ["localhost:443/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'port' => 443, 'path' => '/x.png'])
            ],
            ["http://127.0.0.1/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => '127.0.0.1', 'path' => '/x.png'])
            ],
            ["127.0.0.1/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => '127.0.0.1', 'path' => '/x.png'])
            ],
            ["http://localhost:8080/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'port' => 8080, 'path' => '/x.png'])
            ],
            ["http://sub.example.com/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'sub.example.com', 'path' => '/x.png'])
            ],
            ["http://s-1.d-1.com/x.png",
                array_merge($defaults, ['scheme' => 'http', 'host' => 's-1.d-1.com', 'path' => '/x.png'])
            ],
            ["localhost:443/x.html#somewhere",
                array_merge($defaults, ['scheme' => 'http', 'host' => 'localhost', 'port' => 443, 'path' => '/x.html',
                            'fragment' => 'somewhere'])
            ],
            ["ftp://s-1.d-1.com/x.png", // Ftp:// is supported.
                array_merge($defaults, ['scheme' => 'ftp', 'host' => 's-1.d-1.com', 'path' => '/x.png'])
            ],
            ["http://localhost:0/x.png", $defaults],  // Port 0 is not supported, so parsing will fail.
            ["file://s-1.d-1.com/x.png", $defaults ], // File:// is not supported.
            ["ftp:///s-1.d-1.com/x.png", $defaults],  // Triple slash results in a parsing failure.
            ["dict://s-1.d-1.com/x.png", $defaults],  // Dict:// is not supported.
            ["/somearea/images/x.png", $defaults],    // Paths only are not supported.
            ["//somearea/images/x.png", $defaults],   // Paths are not supported.
            [" /somearea/images/x.png", $defaults],   // Paths are not supported.
            [":/somearea/images/x.png", $defaults],   // Paths are not supported.
        ];
    }
