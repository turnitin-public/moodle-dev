...
define(['jquery', 'core/ajax', 'core/str', 'core/modal_factory', 'core/modal_events', 'core/notification',
    ...
    function($, Ajax, Str, ModalFactory, ModalEvents, Notification, CustomEvents, Templates) {
        ...
        var getIconKeyFromState = function(state, tracking) {
            // To generate the icon keys, one of:
            // 'i/completion-auto-y-override'
            // 'i/completion-auto-n-override'
            // 'i/completion-manual-y-override'
            // 'i/completion-manual-n-override'.
            var key;
            if (tracking == 'auto') {
                key = 'i/completion-auto-';
            } else if (tracking == 'manual') {
                key = 'i/completion-manual-';
            }
            return state > 0 ? key + 'y-override' : key + 'n-override';
        };
