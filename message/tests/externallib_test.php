...
class core_message_externallib_testcase extends externallib_advanced_testcase {
    ...
    /**
     * Test the get_user_favourite_conversations() method.
     */
    public function test_get_user_favourite_conversations() {
        $this->resetAfterTest();
        global $DB;
        ...
        // Initially, there should be no favourite conversations.
        $result = core_message_external::get_favourite_conversations($user1->id);
        $result = external_api::clean_returnvalue(core_message_external::get_favourite_conversations_returns(), $result);
        $messages = $result['contacts'];
        ...
        // Retrieve the favourite conversations for user 1.
        $result = core_message_external::get_favourite_conversations($user1->id);
        ...
        $this->assertFalse($message1['sentfromcurrentuser']);
        $this->assertEquals('Word.', $message1['lastmessage']);
        $this->assertEquals($messageid1, $message1['messageid']);
        $this->assertNull($message1['isonline']);
        $this->assertFalse($message1['isread']);
        $this->assertFalse($message1['isblocked']);
        $this->assertEquals(2, $message1['unreadcount']);

        // Now, verify that user2 has no favourited conversations.
        $this->setUser($user2);
        $result = core_message_external::get_favourite_conversations($user2->id);
        $result = external_api::clean_returnvalue(core_message_external::get_favourite_conversations_returns(), $result);
        $messages = $result['contacts'];
        $this->assertCount(0, $messages);
    }
    ...
    public function test_unset_favourite_conversations_basic() {
        ...
        $this->assertCount(2, \core_message\api::get_favourite_conversations($user1->id));
    ...
    public function test_unset_favourite_conversations_another_users_conversation() {
        ...
        $conversation1 = \core_message\api::get_conversation_between_users([$user1->id, $user2->id]);
        \core_message\api::set_favourite_conversation($conversation1, $user1->id);
        $this->assertCount(1, \core_message\api::get_favourite_conversations($user1->id));
