...
/**
 * Contains the deployment_repository class.
 *
 * @package enrol_lti
...
class deployment_repository {
     ...
     * @return deployment a deployment.
     */
    private function deployment_from_record(\stdClass $record) {
        $deployment = deployment::create_from_store(
            $record->id,
            $record->name,
            $record->deploymentid,
            $record->platformid
     ...
     */
    public function count_by_registration(int $registrationid) {
        global $DB;
