...
var CSS = {
        ...
        OPTION_ALLOW_OFFLINE_BUTTON: 'atto_h5p_option_allow_offline_button',
    ...
    SELECTORS = {
        ...
        OPTION_ALLOW_OFFLINE_BUTTON: '.' + CSS.OPTION_ALLOW_OFFLINE_BUTTON,
                    ...
                    '<fieldset class="collapsible {{#if collapseOptions}}collapsed{{/if}}" id="{{elementid}}_h5poptions">' +
                        ...
                        '<div class="fcontainer">' +
                            ...
                            '<div class="form-check mb-2">' +
                                '<input type="checkbox" {{optionAllowOfflineButton}} ' +
                                'class="form-check-input {{CSS.OPTION_ALLOW_OFFLINE_BUTTON}}" ' +
                                    'id="{{elementid}}_h5p-option-allowoffline-button"/>' +
                                '<label class="form-check-label" for="{{elementid}}_h5p-option-allowoffline-button">' +
                                '{{get_string "allowoffline" component}}' +
                                '</label>' +
                            '</div>' +
                        '</div>' +
                    '</fieldset>' +
...
Y.namespace('M.atto_h5p').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _getDialogueContent: function() {
        ...
        if (this._H5PDiv) {
            ...
            if (fileBaseUrl == H5PURL.substring(0, fileBaseUrl.length)) {
                ...
                if (parameters) {
                    ...
                    if (parameters.match(/offline=1/)) {
                        optionAllowOfflineButton = 'checked';
                        collapseOptions = false;
                    }
                }
            } else {
        ...
        var template = Y.Handlebars.compile(TEMPLATE),
            content = Y.Node.create(template({
                ...
                optionAllowOfflineButton: optionAllowOfflineButton,
    ...
    _setH5P: function(e) {
        ...
        if (url !== '') {
            ...
            if (form.one(SELECTORS.OPTION_ALLOW_OFFLINE_BUTTON).get('checked')) {
                options.offline = '1';
            }
