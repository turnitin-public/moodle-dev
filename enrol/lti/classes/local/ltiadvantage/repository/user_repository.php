...
/**
 * Contains the user_repository class.
 *
 * @package enrol_lti
...
class user_repository {
    ...
    private function user_from_record(\stdClass $userrecord): user {
        ...
        if (isset($userrecord->resourcelinkid)) {
            $user = user::create_from_resource_link(
                $userrecord->resourcelinkid,
                $userrecord->toolid,
                $userrecord->deploymentid,
                $userrecord->sourceid,
    ...
    private function lti_user_record_from_user(user $user): \stdClass {
        $record = [
            ...
            'deploymentid' => $user->get_deploymentid(),
     ...
     */
    private function user_exists_for_tool_and_deployment(user $user): bool {
        // Lack of an id doesn't preclude the object from existence in the store. It may be a stale object.
        // If we can identify the user in the store (unique toolid:userid map), and no id was given, reject the save.
        global $DB;
        $uniquesql = "SELECT lu.id
                        FROM {{$this->ltiuserstable}} lu
                        JOIN {user} u
                          ON (lu.userid = u.id AND lu.toolid = :toolid)
                       WHERE u.username = :username";
        $params = ['username' => $user->get_username(), 'toolid' => $user->get_resourceid()];
    ...
    public function save(user $user): user {
        ...
        if ($exists) {
            ...
            // Validate uniqueness, in the case of a stale object coming in to be saved.
            if ($this->user_exists_for_tool_and_deployment($user)) {
                throw new \coding_exception("Cannot create duplicate user {$user->get_username()} for resource " .
                    "{$user->get_resourceid()}.");
            }

            // The user record is not deleted at any time by the tool, despite the enrol_lti_users record being removed.
            // If saving an lti user and the user is pre-existing, re-link it.
            $userrecord->timecreated = $userrecord->timemodified = $timenow;
            if ($userrec = $DB->get_record('user', ['username' => $user->get_username()], 'id')) {
                $userid = $userrec->id;
                $userrecord->id = $userid;
                \user_update_user($userrecord);
                unset ($userrecord->id);
        ...
        if ($reslinkid = $user->get_resourcelinkid()) {
            $resourcelinkmap = ['userid' => $ltiuserrecord->id, 'resourcelinkid' => $reslinkid];
        ...
        $record = (object) array_merge((array) $userrecord, (array) $ltiuserrecord, $resourcelinkmap,
            ['localid' => $userid]);
    ...
    /**
     * Find an lti user by their username and for a given resource/enrolment method.
     *
     * The same user (sourceid) can originate from multiple resource links for multiple resources, so to get a
     * contextual user record, resourceid is required.
     *
     * @param string $username the string username identifying the user.
     * @param int $resourceid the id of the published resource to scope the user to.
     * @return user|null the user instance or null if not found.
     */
    public function find_by_username(string $username, int $resourceid): ?user {
        global $DB;
        try {
            $sql = "SELECT lu.id, u.id as localid, u.username, u.firstname, u.lastname, u.email, u.city, u.country,
                           u.institution, u.timezone, u.maildisplay, u.lang, lu.sourceid, lu.toolid, lu.lastgrade,
            ...
            $record = $DB->get_record_sql($sql, ['username' => $username, 'resourceid' => $resourceid], MUST_EXIST);
            return $this->user_from_record($record);
        } catch (\dml_missing_record_exception $ex) {
            return null;
        }
    ...
    public function delete(int $id) {
        ...
        $DB->delete_records($this->userresourcelinkidtable, ['userid' => $id]);
    }
    ...
    public function delete_by_deployment(int $deploymentid): void {
        global $DB;
        $DB->delete_records($this->ltiuserstable, ['deploymentid' => $deploymentid]);
    }
}
