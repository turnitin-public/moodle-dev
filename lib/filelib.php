...
class curl {
    ...
    /** @var bool whether or not to blacklist is enabled at site level.*/
    ...
    public function __construct($settings = array()) {
        ...
        if (\core\curl_security::is_enabled() && empty($settings['ignoreblacklist'])) {
    ...
    protected function request($url, $options = array()) {
        ...
        if ($this->blacklistenabled && \core\curl_security::url_is_blacklisted($url)) {
            $this->error = \core\curl_security::get_blocked_url_string();
            return $this->error;
        }
        ...
        if ($this->blacklistenabled && intval($this->info['redirect_count']) > 0) {
            if (\core\curl_security::url_is_blacklisted($this->info['url'])) {
                $this->reset_request_state_vars();
                $this->error = \core\curl_security::get_blocked_url_string();
                curl_close($curl);
                return $this->error;
