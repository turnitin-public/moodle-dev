...
$ADMIN->add('enrolments', new admin_category('enrolltifolder', new lang_string('pluginname', 'enrol_lti'), $this->is_enabled() === false));
...
$settings = new admin_settingpage('enrolsettingslti_registrations', "Tool registration", 'moodle/site:config', $this->is_enabled() === false);

$settings->add(new admin_setting_heading('enrol_lti_tool_endpoints_heading',
    get_string('toolendpoints', 'enrol_lti'), ''));
$settings->add(new enrol_lti\admin_setting_toolendpoints());

$settings->add(new admin_setting_heading('enrol_lti_tool_registrations_heading',
    get_string('registeredplatforms', 'enrol_lti'), ''));
$settings->add(new enrol_lti\admin_setting_registeredplatforms());

$ADMIN->add('enrolltifolder', $settings);


// TODO: Lang strings
// This adds a settings page to the 'publish as lit tool' folder, hidden.
// On this page, we'll  override the active node to force a match on enrolsettingslti_registrations settings page.
$ADMIN->add('enrolltifolder', new admin_externalpage('enrolsettingslti_registrations_edit', "Register a platform", "$CFG->wwwroot/$CFG->admin/enrol/lti/register_platform.php", 'moodle/site:config', true));


/*
// Create a standalone registrations page.
$settings = new admin_externalpage('ltitoolregistration',
    "Tool registration",
    new moodle_url('/enrol/lti/tool_details.php'));
$ADMIN->add('enrolltifolder', $settings);
*/
