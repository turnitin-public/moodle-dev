...
class course_tools_page_header implements \templatable {
    ...
    public function export_for_template(renderer_base $output): \stdClass {
        ...
        if ($this->canadd) {
            $context->addlink = (new \moodle_url('/ltix/coursetooledit.php', ['course' => $this->courseid]))->out();
        }

        if ($this->toolcount == 0) {
            $notification = new notification(get_string('nocourseexternaltoolsnotice', 'core_ltix'), notification::NOTIFY_INFO, true);
