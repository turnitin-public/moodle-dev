...
function get_profile_roles(context $context) {
    ...
    $rolesinscope = explode(',', $CFG->profileroles);
    if (has_capability('moodle/role:assign', $context)) {
        $assignablerolesassigned = array_filter(get_assignable_roles($context, ROLENAME_ALIAS, true)[1], function($val) {
            return $val > 0;
        });
        $rolesinscope = array_unique(array_merge($rolesinscope, array_keys($assignablerolesassigned)), SORT_REGULAR);
    }
