...
if ($legacy) {
    ...
    echo $OUTPUT->heading(get_string('publishedresources', 'enrol_lti'));
    echo "<p>" . get_string('publishedresources_help', 'enrol_lti') . "</p>";
    echo "<p class=helplink>" . $OUTPUT->doc_link('enrol/lti/index',
            get_string('morehelp')) . "</p>";
}
...
if ($legacy) {
    ...
    if (\enrol_lti\helper::count_lti_tools(array('courseid' => $courseid, 'ltiversion' => 'LTI-1p3')) > 0) {
        ...
        $notify = new \core\output\notification(get_string('nopublishedresources', 'enrol_lti'),
            \core\output\notification::NOTIFY_INFO);
        echo $OUTPUT->render($notify);
    }
}
