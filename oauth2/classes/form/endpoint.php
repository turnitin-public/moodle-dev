...
class endpoint extends persistent {
    ...
    public function definition() {
        ...
        // Name.
        $mform->addElement('text', 'name', get_string('endpointname', 'oauth2'));
        $mform->addRule('name', null, 'required', null, 'client');
        $mform->addRule('name', get_string('maximumchars', '', 255), 'maxlength', 255, 'client');
        $mform->addHelpButton('name', 'endpointname', 'oauth2');

        // Url.
        $mform->addElement('text', 'url', get_string('endpointurl', 'oauth2'));
        $mform->addRule('url', null, 'required', null, 'client');
        $mform->addRule('url', get_string('maximumchars', '', 1024), 'maxlength', 1024, 'client');
        $mform->addHelpButton('url', 'endpointurl', 'oauth2');
