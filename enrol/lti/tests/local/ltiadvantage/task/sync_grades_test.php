...
class sync_grades_test extends \lti_advantage_testcase {
    ...
    protected function get_task_with_mocked_grade_service($statuscode = '200', $mockexception = false): sync_grades {
        $mockgradeservice = $this->getMockBuilder(LTI_Assignments_Grades_Service::class)
            ->disableOriginalConstructor()
            ->onlyMethods(['put_grade'])
            ->getMock();
        $mockgradeservice->method('put_grade')->willReturnCallback(function() use ($statuscode, $mockexception) {
            if ($mockexception) {
                throw new \Exception();
            }
            return ['headers' => ["HTTP/2 $statuscode OK"]];
        });
