...
class mustache_template_source_loader {
     ...
     * @return string
     */
    public function strip_template_comments($templatestr) : string {
        return preg_replace('/(?={{!)(.*)(}})/sU', '', $templatestr);
    }

    /**
     * Load a template's source using the full template name (e.g. <component>/<template_name>) and theme
     * name (e.g. boost).
     *
     * @param string $fulltemplatename The the component and template name combined (e.g. core_message/message_drawer)
     ...
     */
    public function load_from_full_name(
        string $fulltemplatename,
        string $themename,
        bool $includecomments = false
    ) : string {
        // Get the component and name from the full name (e.g. core_message/message_drawer).
        list($component, $name) = explode('/', $fulltemplatename);
        return $this->load($component, $name, $themename, $includecomments);
    }
    ...
    public function load_with_dependencies(
        ...
        $seentemplates = $handler(
            ...
            // outside of this anonymous function.
            function($component, $id) use ($themename, $includecomments, &$seentemplates, &$seenstrings, &$templates, &$strings, $save) {
     ...
     * @return array
     */
    public function scan_template_source_for_dependencies(string $source) : array {
